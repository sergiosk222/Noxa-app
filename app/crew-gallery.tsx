...
type CrewRow = { id: string; name: string; owner_id: string };
type MembershipRow = { role: "owner" | "admin" | "member" };
type ProfileRow = {
  id: string;
...
export default function CrewGalleryScreen() {
  ...
  const canManage = useMemo(
    () =>
      ...
      || Boolean(crew && currentUserId === crew.owner_id),
    [crew, currentUserId, membershipRole],
  );
  ...
  const loadItems = useCallback(async () => {
    ...
    const { data, error: itemsError } = await supabase
      ...
      .select("id,crew_id,uploader_id,object_path,caption,created_at")
      .eq("crew_id", crewId)
  ...
  const loadGallery = useCallback(async () => {
    ...
    if (authError || !userId) {
      setError("Sign in to open the crew gallery.");
      setLoading(false);
      return;
    }
    ...
    const role = membershipResult.error
      ? null
      : ((membershipResult.data as MembershipRow | null)?.role ?? null);
    setMembershipRole(role);
    setCanUpload(Boolean(role));
    await loadItems();
    setLoading(false);
  }, [crewId, loadItems]);
  ...
  const chooseImage = useCallback(async () => {
    ...
    if (!permission.granted) {
      Alert.alert(
        "Photo access needed",
        "Allow photo library access to add a crew photo.",
      );
      return;
    }
  ...
  const uploadImage = useCallback(async () => {
    ...
    try {
      ...
      uploadedPath = `${currentUserId}/${crewId}/${Date.now()}-${suffix}.${extension}`;
      ...
      const { error: rowError } = await supabase.from("crew_gallery_items").insert({
        crew_id: crewId,
      ...
      if (uploadedPath) {
        await supabase.storage.from(crewGalleryBucket).remove([uploadedPath]);
      }
      setError(
  ...
  }, [caption, currentUserId, crewId, loadItems, pendingAsset, uploading]);

  const confirmDelete = useCallback(() => {
    if (!selectedItem || deleting) return;
    Alert.alert("Remove photo?", "This removes the photo from the crew gallery.", [
    ...
    <NoxaScreen padded={false}>
      <View style={styles.header}>
        ...
        <View style={styles.headerCopy}>
          <Text style={styles.headerTitle}>CREW GALLERY</Text>
          <Text numberOfLines={1} style={styles.headerSubtitle}>{crew?.name ?? "NOXA crew"}</Text>
        </View>
        <Text style={styles.photoCount}>{items.length} PHOTOS</Text>
        {canUpload ? (
          <Pressable
            accessibilityLabel="Add crew photo"
            accessibilityRole="button"
      ...
      {loading ? (
        <View style={styles.state}>
          ...
          <Text style={styles.stateText}>Loading crew photos…</Text>
        </View>
      ) : (
        <ScrollView
        ...
        >
          ...
          {pendingAsset ? (
            <View style={styles.uploadCard}>
              ...
              <View style={styles.uploadFields}>
                <Text style={styles.uploadTitle}>NEW CREW PHOTO</Text>
                <TextInput
                  value={caption}
          ...
          {items.length === 0 ? (
            <View style={styles.empty}>
              ...
              <Text style={styles.emptyTitle}>NO CREW PHOTOS YET</Text>
              <Text style={styles.emptyText}>
                {canUpload
                  ? "Add the first real moment from this crew."
                  : "Crew member photos will appear here after you join."}
              </Text>
              {canUpload ? (
      ...
      >
        <View style={styles.lightbox}>
          <View style={styles.lightboxHeader}>
            ...
            <Text style={styles.lightboxTitle}>CREW PHOTO</Text>
            {selectedItem
            && (selectedItem.uploader_id === currentUserId || canManage) ? (
              <Pressable
